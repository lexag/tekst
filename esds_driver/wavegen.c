...
static int current_red = 0;
static int current_green = 0;
...
static inline void set_pin(int pin, int set) {
  uint8_t mask = 1u << pin;
  state = (state & ~mask) | (set * mask);
}

static inline void clk(int set) { set_pin(pin(CLK), set); }

void writestd(uint8_t out[]) {
  out[wp] = state;
  wp++;
  out[wp] = state;
  wp++;
