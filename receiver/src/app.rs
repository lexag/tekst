use crate::{handler::Handler, DISPLAY_HEIGHT, DISPLAY_WIDTH};
            ...
            for y in 0..DISPLAY_HEIGHT {
                for x in 0..DISPLAY_WIDTH {
                    let is_red = self.handler.display.reds.bits[x / 8 + y * DISPLAY_WIDTH / 8]
                        & 0x1 << (7 - x % 8)
                        != 0;
                    let is_green = self.handler.display.greens.bits[x / 8 + y * DISPLAY_WIDTH / 8]
                        & 0x1 << (7 - x % 8)
                        != 0;
                    ...
                    p.circle_filled(
                        resp.rect.min + vec2(POINT_SIZE * x as f32, POINT_SIZE * y as f32),
                        POINT_SIZE * 0.5,
                        col,
                    );
                }
